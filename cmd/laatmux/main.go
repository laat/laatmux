...
// the current tmux session if none exists. Local and remote are the same
...
func cmdJump(ctx context.Context, args []string) error {
	...
	local := tmux.Server{}
	tag := h.Name + "/" + session
	out, err := local.Run(ctx, "list-panes", "-a", "-F", strings.Join([]string{"#{@laatmux_attach}", "#{session_name}", "#{window_id}", "#{pane_id}"}, tmux.Sep))
	if err != nil {
		return err
	}
	...
	attach := tmux.LaatmuxServer.AttachArgs(session)
	var shellCmd string
	if h.Local() {
		shellCmd = tmux.ShellJoin(attach)
	} else {
		shellCmd = tmux.ShellJoin([]string{"ssh", "-t", h.SSH, tmux.ShellJoin(attach)})
	}
	out, err = local.Run(ctx, "new-window", "-n", session, "-P", "-F", "#{pane_id}", shellCmd)
	if err != nil {
		return err
	}
